...
///
/// Pass 1 — Ocean mask:        world noise value vs OceanLevel
...
public static class WorldGenerator
{
    ...
    /// worldNoiseHeight is the raw noise value at this position BEFORE
    /// any overlays — used for the ocean mask.
    /// </summary>
    public static BiomeSample Sample(Vector2 worldPos, WorldConfig config, float worldNoiseHeight)
    {
        // Pass 1 — Ocean mask
        if (worldNoiseHeight < config.OceanLevel)
        {
        ...
        // Pass 2 + 3 — Voronoi with distortion (Pass 4 border detection inline)
    ...
    static BiomeSample SampleVoronoi(Vector2 worldPos, WorldConfig config)
    {
        ...
        float distX = (Mathf.PerlinNoise(worldPos.x * distFreq, worldPos.y * distFreq) - 0.5f)
                      * config.BiomeDistortionStrength;
        float distY = (Mathf.PerlinNoise((worldPos.x + 1739f) * distFreq, (worldPos.y + 3319f) * distFreq) - 0.5f)
                      * config.BiomeDistortionStrength;
        Vector2 distortedPos = worldPos + new Vector2(distX, distY);

        // Cell the distorted position falls in
    ...
    /// <summary>
    /// Returns the random point inside a cell using the video's hash approach.
    /// </summary>
    ...
    /// <summary>
    /// Resolves a cell's climate coords and returns the nearest BiomeEntry's Config.
    /// </summary>
    static LevelGeneratorCommon GetBiomeConfig(int cellX, int cellY, float cellSize, WorldConfig config)
    {
        ...
        // Pass 5 — sample climate at cell center
